...
//    }
    ...
    public static LinkedList<Integer> getLinkedKey(int[] a, HashMap<Integer, LinkedList<Integer>> hashMap, int initialIndex) {
        int index = find(a, initialIndex);
...
//    public static void deleteSetsUnderSize(int[] a, int minSize) {
...
//        }
    ...
    public static void deleteSet(int[] a, HashMap<Integer, LinkedList<Integer>> hashMap, HashMap<Integer, LinkedList<Integer>> hashMapLinked, int index) {
        if (hashMapLinked != hashMap) {
            LinkedList<Integer> set = hashMap.get(DisjointSetFunctions.find(a, index));
            LinkedList<Integer> cluster = DisjointSetFunctions.getLinkedKey(a, hashMapLinked, index);
            int key = DisjointSetFunctions.getKey(a, hashMapLinked, index);

            if (cluster != null) {
                cluster.removeAll(set);
                if (cluster.isEmpty())
                    hashMapLinked.remove(key);
            }
        }

        hashMap.remove(find(a, index));
    }
